...
data "vault_generic_secret" "pm_api_token_id" {
  path = "secret/SECRETKEY/TIBRAHIM"
}

data "vault_generic_secret" "pm_api_token_secret" {
  path = "secret/ACCESSKEY/TIBRAHIM"
}
...
###############################################################################
# Terraform Plan for Database instances
###############################################################################
resource "proxmox_vm_qemu" "database" {
  count       = var.database-numberofvms
  name        = "${var.database-yourinitials}-vm${count.index}.service.consul"
  desc        = var.database-desc
  #target_node = data.vault_generic_secret.target_node.data[random_shuffle.nodename.result[0]]
  target_node = random_shuffle.nodename.result[0]
  clone       = var.database-template_to_clone
  os_type     = "cloud-init"
  memory      = var.database-memory
  cores       = var.database-cores
  sockets     = var.database-sockets
  scsihw      = "virtio-scsi-pci"
  boot        = "order=virtio0"
  agent       = 1
  tags        = var.be-tags
  ...
  provisioner "remote-exec" {
    ...
    # instance and configure the system to register the FQDN with the Consul DNS system
    inline = [
      "sudo hostnamectl set-hostname ${var.database-yourinitials}-vm${count.index}",
      "sudo sed -i 's/changeme/${random_id.id.dec}${count.index}/' /etc/consul.d/system.hcl",
      "sudo sed -i 's/replace-name/${var.database-yourinitials}-vm${count.index}/' /etc/consul.d/system.hcl",
      "sudo sed -i 's/ubuntu-server/${var.database-yourinitials}-vm${count.index}/' /etc/hosts",
      "sudo sed -i 's/FQDN/${var.database-yourinitials}-vm${count.index}.service.consul/' /etc/update-motd.d/999-consul-dns-message",
      "sudo sed -i 's/#datacenter = \"my-dc-1\"/datacenter = \"rice-dc-1\"/' /etc/consul.d/consul.hcl",
      "echo 'retry_join = [\"${var.consulip-240-prod-system28}\",\"${var.consulip-240-student-system41}\",\"${var.consulip-242-room}\"]' | sudo tee -a /etc/consul.d/consul.hcl",
      ...
      "sudo rm /opt/consul/node-id",
      "sudo systemctl restart consul.service",
      "sudo sed -i 's/0.0.0.0/${var.database-yourinitials}-vm${count.index}.service.consul/' /etc/systemd/system/node-exporter.service",
      "sudo systemctl daemon-reload",
      "sudo systemctl enable node-exporter.service",
      "sudo systemctl start node-exporter.service",
      "sudo systemctl stop mariadb.service",
      "sudo sed -i '1,$s/127.0.0.1/${var.database-yourinitials}-vm${count.index}.service.consul/g' /etc/mysql/mariadb.conf.d/50-server.cnf",
      "sudo systemctl daemon-reload",
...
output "proxmox_database_ip_address_default" {
  description = "Current Public IP"
